...
class core_config {
  public:
    ...
    std::uint32_t
        max_work_group_size;         ///< Maximum size of local work group in number of works items
...
struct tinytc_core_info : tinytc::reference_counted {
    ...
    virtual auto register_space() const -> std::uint32_t = 0;
    //! Get core features
    ...
    virtual auto minmax_work_group_size() const -> std::uint32_t = 0;
...
class core_info_generic : public ::tinytc_core_info {
  public:
    core_info_generic(std::uint32_t register_space, std::uint32_t max_work_group_size,
                      std::vector<std::uint32_t> subgroup_sizes);
    auto subgroup_sizes() const -> std::vector<std::uint32_t> const & override;
    auto register_space() const -> std::uint32_t override;
    ...
    auto minmax_work_group_size() const -> std::uint32_t override;
    auto get_core_config(std::uint32_t subgroup_size) const -> tinytc::core_config override;

  private:
    std::uint32_t register_space_;
    std::uint32_t max_work_group_size_;
...
class core_info_intel : public ::tinytc_core_info {
  public:
    ...
    core_info_intel(std::uint32_t ip_version, std::uint32_t num_eus_per_subslice,
                    std::uint32_t num_threads_per_eu, std::vector<std::uint32_t> subgroup_sizes);
  ...
  private:
    ...
    auto max_work_group_size(std::uint32_t subgroup_size) const -> std::uint32_t;
