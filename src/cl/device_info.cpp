...
tinytc_status_t tinytc_cl_core_info_create(tinytc_core_info_t *info, cl_device_id device) {
    ...
    TINYTC_CL_CHECK(clGetDeviceInfo(device, CL_DEVICE_NUM_EUS_PER_SUB_SLICE_INTEL,
                                    sizeof(num_eus_per_subslice), &num_eus_per_subslice, nullptr));
    TINYTC_CL_CHECK(clGetDeviceInfo(device, CL_DEVICE_NUM_THREADS_PER_EU_INTEL,
                                    sizeof(num_threads_per_eu), &num_threads_per_eu, nullptr));
    TINYTC_CL_CHECK(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(local_mem_size),
    ...
    TINYTC_CL_CHECK(clGetDeviceInfo(device, CL_DEVICE_SUB_GROUP_SIZES_INTEL, subgroup_sizes_size,
