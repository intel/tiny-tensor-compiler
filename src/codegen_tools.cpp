...
void tile_loop_by_sgs_new_constant(region_builder &bb, std::int64_t loop_trip_count, int sgs,
    ...
    if (blocks > 0) {
        ...
        bb.for_loop(std::move(block_start), c_sgs_blocks, c_sgs_tiles, index_ty,
...
void tile_loop_uniformly_new_constant(region_builder &bb, std::int64_t loop_trip_count,
    ...
    if (rem > 0) {
        ...
        bb.for_loop(std::move(block_start), c_bs_1_rem, c_bs_1_tiles, index_ty,
                    [&](region_builder &bb, value block) { body(bb, block, c_bs_1); });
    }
    ...
    bb.for_loop(std::move(block_start), c_loop_trip_count, c_bs_tiles, index_ty,
                [&](region_builder &bb, value block) { body(bb, block, c_bs); });
}
