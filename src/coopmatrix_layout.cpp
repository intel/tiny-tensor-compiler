...
auto get_layout(core_config const &cfg, coopmatrix_type const *ct) -> coopmatrix_layout {
    ...
    l.shape1 = ct->cols();
    l.blocks1 = 1;
    auto cty_size = size(l.cty);
    if (ct->use() == matrix_use::b && l.blocks > 1) {
        const auto omega_b = std::max(1, static_cast<int>(2 / cty_size));
        l.blocks1 = omega_b;
    }
    l.ops_per_chan = 1;
    if (ct->use() == matrix_use::a) {
        const auto omega = std::max(1, static_cast<int>(4 / cty_size));
