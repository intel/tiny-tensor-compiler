...
/* region builder */
region_builder::region_builder() : region_(std::make_shared<internal::rgn>()) {}
region_builder::region_builder(std::shared_ptr<internal::rgn> region)
    : region_(std::move(region)) {}

region region_builder::get_product() { return region(region_); }

value region_builder::create_binary_op(binary_op op, value a, value b, std::string const &prefix,
                                       std::source_location const loc) {
    return insert(std::make_shared<internal::binary_op_inst>(op, std::move(a), std::move(b),
                                                             to_location(loc)),
                  prefix);
}
...
value region_builder::create_cast(value a, scalar_type to_ty, std::string const &prefix,
                                  std::source_location const loc) {
    return insert(std::make_shared<internal::cast_inst>(std::move(a), to_ty, to_location(loc)),
                  prefix);
}

value region_builder::create_cmp(cmp_condition cond, value a, value b, std::string const &prefix,
                                 std::source_location const loc) {
    return insert(std::make_shared<internal::compare_inst>(cond, std::move(a), std::move(b),
                                                           to_location(loc)),
                  prefix);
}
...
value region_builder::create_neg(value a, std::string const &prefix,
                                 std::source_location const loc) {
    return insert(std::make_shared<internal::neg_inst>(std::move(a), to_location(loc)), prefix);
}

value region_builder::create_alloca(data_type ty, std::string const &prefix,
                                    std::source_location const loc) {
    return insert(std::make_shared<internal::alloca_inst>(std::move(ty), to_location(loc)), prefix);
}
...
void region_builder::create_axpby(transpose tA, value alpha, value A, value beta, value B,
    ...
    insert(std::make_shared<internal::axpby_inst>(tA, std::move(alpha), std::move(A),
                                                  std::move(beta), std::move(B), atomic,
                                                  to_location(loc)));
}

void region_builder::create_barrier(std::source_location const loc) {
    auto b = std::make_shared<internal::barrier_inst>();
...
value region_builder::create_expand(value arg, std::int64_t mode, std::vector<value> expand_shape,
    ...
    return insert(std::make_shared<internal::expand_inst>(
                      std::move(arg), mode, std::move(expand_shape), to_location(loc)),
                  prefix);
}

value region_builder::create_fuse(value arg, std::int64_t from, std::int64_t to,
                                  std::string const &prefix, std::source_location const loc) {
    return insert(std::make_shared<internal::fuse_inst>(std::move(arg), from, to, to_location(loc)),
                  prefix);
}

value region_builder::create_load(value arg, std::vector<value> index_list,
                                  std::string const &prefix, std::source_location const loc) {
    return insert(std::make_shared<internal::load_inst>(std::move(arg), std::move(index_list),
                                                        to_location(loc)),
                  prefix);
}

value region_builder::create_group_id(std::string const &prefix, std::source_location const loc) {
    auto g = std::make_shared<internal::group_id_inst>();
...
value region_builder::create_group_size(std::string const &prefix, std::source_location const loc) {
    auto g = std::make_shared<internal::group_size_inst>();
    g->loc(to_location(loc));
    return insert(std::move(g), prefix);
}
...
void region_builder::create_gemm(transpose tA, transpose tB, value alpha, value A, value B,
    ...
    insert(std::make_shared<internal::gemm_inst>(tA, tB, std::move(alpha), std::move(A),
                                                 std::move(B), std::move(beta), std::move(C),
                                                 atomic, to_location(loc)));
}

void region_builder::create_gemv(transpose tA, value alpha, value A, value B, value beta, value C,
                                 bool atomic, std::source_location const loc) {
    insert(std::make_shared<internal::gemv_inst>(tA, std::move(alpha), std::move(A), std::move(B),
...
void region_builder::create_ger(value alpha, value A, value B, value beta, value C, bool atomic,
                                std::source_location const loc) {
    insert(std::make_shared<internal::ger_inst>(std::move(alpha), std::move(A), std::move(B),
                                                std::move(beta), std::move(C), atomic,
                                                to_location(loc)));
}
...
void region_builder::create_hadamard(value alpha, value A, value B, value beta, value C,
                                     bool atomic, std::source_location const loc) {
    insert(std::make_shared<internal::hadamard_inst>(std::move(alpha), std::move(A), std::move(B),
                                                     std::move(beta), std::move(C), atomic,
                                                     to_location(loc)));
}
...
value region_builder::create_size(value arg, std::int64_t mode, std::string const &prefix,
                                  std::source_location const loc) {
    return insert(std::make_shared<internal::size_inst>(std::move(arg), mode, to_location(loc)),
                  prefix);
}

value region_builder::create_subview(value op, std::vector<slice> slices, std::string const &prefix,
                                     std::source_location const loc) {
    return insert(std::make_shared<internal::subview_inst>(std::move(op), std::move(slices),
                                                           to_location(loc)),
                  prefix);
}
...
void region_builder::create_store(value val, value op, std::vector<value> index_list,
                                  std::string const &prefix, std::source_location const loc) {
    insert(std::make_shared<internal::store_inst>(std::move(val), std::move(op),
                                                  std::move(index_list), to_location(loc)),
           prefix);
}

void region_builder::create_sum(transpose tA, value alpha, value A, value beta, value B,
                                bool atomic, std::source_location const loc) {
    insert(std::make_shared<internal::sum_inst>(tA, std::move(alpha), std::move(A), std::move(beta),
                                                std::move(B), atomic, to_location(loc)));
}

void region_builder::create_for(value loop_var, value from, value to, region body,
                                std::source_location const loc) {
    auto f = std::make_shared<internal::for_inst>(std::move(loop_var), std::move(from),
                                                  std::move(to), std::move(body), to_location(loc));
    insert(std::move(f));
}
void region_builder::create_for(value loop_var, value from, value to, value step, region body,
                                std::source_location const loc) {
    auto f =
        std::make_shared<internal::for_inst>(std::move(loop_var), std::move(from), std::move(to),
...
void region_builder::create_foreach(value loop_var, value from, value to, region body,
                                    std::source_location const loc) {
    auto f = std::make_shared<internal::foreach_inst>(
        std::move(loop_var), std::move(from), std::move(to), std::move(body), to_location(loc));
    insert(std::move(f));
}

void region_builder::create_if(value condition, region then, region otherwise,
                               std::source_location const loc) {
    auto i = std::make_shared<internal::if_inst>(std::move(condition), std::move(then),
                                                 std::move(otherwise));
    i->loc(to_location(loc));
    insert(std::move(i));
}

value region_builder::insert(std::shared_ptr<internal::inst_node> s, std::string const &prefix) {
    auto result = s->result();
    region_->insts().emplace_back(inst(std::move(s)));
...
func function_builder::get_product() {
    if (!body_.get()) {
        body_ = region(std::make_shared<internal::rgn>());
    }
    return func(std::make_shared<internal::function>(func(proto_), body_, work_group_size_,
                                                     subgroup_size_));
}
...
value function_builder::argument(data_type ty, std::string const &prefix,
    ...
    proto_->args().emplace_back(std::move(v));
    return proto_->args().back();
}

/* program builder */
void program_builder::add(func f) { functions_.emplace_back(std::move(f)); }

prog program_builder::get_product() {
    return prog(std::make_shared<internal::program>(functions_));
}
