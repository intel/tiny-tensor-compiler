...
TEST_CASE_TEMPLATE("sycl packed alpha=1 beta=0", T, TEST_PRECISIONS) {
    ...
    check_small_gemm_batched<T, sycl_test_runtime>(transpose::N, transpose::N, M, N, K, M, M * K, K,
                                                   K * N, M, M * N, 1.0, 0.0, howmany);
}
...
TEST_CASE_TEMPLATE("sycl non-packed alpha=1 beta=0", T, TEST_PRECISIONS) {
    ...
    check_small_gemm_batched<T, sycl_test_runtime>(transpose::N, transpose::N, M, N, K, ldA,
...
TEST_CASE_TEMPLATE("sycl non-packed alpha=1 beta=0 transA transB", T, TEST_PRECISIONS) {
    ...
    check_small_gemm_batched<T, sycl_test_runtime>(transpose::T, transpose::T, M, N, K, ldA,
                                                   ldA * ldB, ldB, ldB * 2 * N, ldC, ldC * 3 * N,
                                                   1.0, 0.0, howmany);
}
